...
figfolder <- paste0("C:/regdata/ethirgast/fig/")
if (!dir.exists(figfolder)) {
  dir.create(figfolder)
}
tabfolder <- paste0("C:/regdata/ethirgast/tab/")
if (!dir.exists(tabfolder)) {
  dir.create(tabfolder)
}
write.csv(regdata_all, paste0(tabfolder, "ethirgastdata.csv"), row.names = FALSE)

...
kjonn_opgr <- regdata |>  dplyr::filter(operation_group != "Other",
  ...
  dplyr::mutate(var = paste0(sprintf("%.1f", men/N*100), " % (N=", N, ")")) |>
...
# print(xtable::xtable(kjonn_opgr, digits=0,
#                      caption='Proportion of men in each operation and age group'),
...
print(xtable::xtable(
  ...
  caption = "N is the number of included patients with a minimum of 110 days follow-up time. unknown is the number of patients that are lost to follow-up at 90 days (not confirmed alive or dead)."),
  include.rownames=FALSE)
...
print(xtable::xtable(
  ...
  caption = "Elective. N is the number of included patients with a minimum of 110 days follow-up time. unknown is the number of patients that are lost to follow-up at 90 days (not confirmed alive or dead)."),
  include.rownames=FALSE)
...
print(xtable::xtable(
  ...
  digits=0,
  align = c("l", "L{1.5in}", rep('r', ncol(tab_deceased90)-2)),
  caption = "Emergency surgery. N is the number of included patients with a minimum of 110 days follow-up time. unknown is the number of patients that are lost to follow-up at 90 days (not confirmed alive or dead)."),
  include.rownames=FALSE)
...
followup_v2 <- regdata %>%
  ...
  dplyr::summarise(
    N = n(),
    N_complete_followup = sum(STATUS_READMISSION90 == 1 |
                                IN_HOUSE_DEATH == 1 |
                                IN_HOUSE_DEATH_READMISSION == 1, na.rm = TRUE),
    N_followed_up = sum((FOLLOWUP_STATUS %in% 1:3 &

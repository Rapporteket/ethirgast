...
registration <-
  ...
  mutate(
    ...
    #   substr(NCSP, 1, 3) == "JLC" ~ "Pancreas resection",
    #   substr(NCSP, 1, 3) == "JJB" ~ "Liver resection",
    #   substr(NCSP, 1, 3) == "JKD" ~ "Biliodigestive anastomosis",
    #   .default = "Other"),
    operation_group = case_when(
      ...
      substr(NCSP, 1, 3) == "JKD" & substr(ICD10, 1, 1) %in% "K" ~
        "Biliodigestive anastomosis, K diagnosis",
      substr(NCSP, 1, 3) == "JKD" & !(substr(ICD10, 1, 1) %in% c("C", "D", "K")) ~
        "Biliodigestive anastomosis, other diagnosis",
      .default = "Other"),
  )
...
summary_stats_all_tr <- summary_stats_all |> tr_summarize_output()
